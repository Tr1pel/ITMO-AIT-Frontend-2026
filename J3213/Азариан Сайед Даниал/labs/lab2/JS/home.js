  ...
  buyButtons.forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        const nextUser = {
          ...baseUser,
          tickets: [ticket, ...(Array.isArray(baseUser.tickets) ? baseUser.tickets : [])],
          refunds: Array.isArray(baseUser.refunds) ? baseUser.refunds : [],
          organizerEvents: Array.isArray(baseUser.organizerEvents) ? baseUser.organizerEvents : [],
        };

        const syncedUser = await persistUserToApi(nextUser);
        users[userIndex] = syncedUser;
        saveUsers(users);

        showPurchaseToast("Билет куплен.", true);
      } catch (error) {
        showPurchaseToast(error.message || "Не удалось купить билет. Попробуйте позже.", false);
      } finally {
