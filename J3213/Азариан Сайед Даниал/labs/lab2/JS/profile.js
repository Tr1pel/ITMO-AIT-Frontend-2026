function initProfilePage() {
  ...
  if (refundForm && refundReason) {
    refundForm.addEventListener("submit", async (event) => {
      ...
      const previousCanRefund = ticket.canRefund;
      const previousRefunds = Array.isArray(currentUser.refunds) ? [...currentUser.refunds] : [];
      ...
      try {
        ...
        ticket.canRefund = previousCanRefund;
        currentUser.refunds = previousRefunds;
        alert(error.message || "Не удалось оформить возврат. Попробуйте позже.");
      } finally {
        if (submitButton) {
